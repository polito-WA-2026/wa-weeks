...
function Database(dbname) {
      ...
      .then(student => {
        ...
        crypto.scrypt(password, student.salt, 32, (err, hash) => {
          ...
          if (crypto.timingSafeEqual(hash, Buffer.from(student.hash, "hex")))
            resolve({id: student.id, username: student.email, name: student.name, secret: student.secret, lastTotpStep: student.lastTotpStep, fullTime: student.full_time === null ? null : Boolean(student.full_time)}); // Avoid full_time = null being cast to false
          else resolve(false);
        });
      })
      .catch(e => reject(e));
  ...
  this.getStudent = async id => {
    const student = await dbGetAsync(
      this.db,
      "select email as username, name, full_time as fullTime, secret, lastTotpStep from students where id = ?",
      [id]
    );
  ...
  this.updateLastTotpStep = (userId, lastTotpStep) => {
    const p = dbRunAsync(this.db, "update students set lastTotpStep = ? where id = ?", [lastTotpStep, userId]);
