...
import LocalStrategy from 'passport-local'; // username and password for login
...
import userDao from './dao-users.mjs'; // module for accessing the user info in the DB

const answerDelay = 1000;  // To be put to 0 for the exam submission
...
// Only users which authenticated with TOTP (and are enabled to do so) can delete their data
app.delete('/api/answers/:id', isLoggedIn, isTotp, [
  check('id').isInt()
], async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
